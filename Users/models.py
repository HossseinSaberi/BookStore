...
class Author(models.Model):
    ...
    author_name = models.CharField(_("Author_Name"), max_length=50)
    ...
    def save(self, *args, **kwargs):
        ...
        self.avatar.name = CUtils.get_upload_path(self.__class__.__name__, self.author_name, file_ext)
        super().save(*args, **kwargs)
        
    

class Translator(models.Model):
    translator_name = models.CharField(_("Translator_Name"), max_length=50, null=True, blank=True)
...
class Profile(CModel.BaseClass):
    ...
    age = models.PositiveIntegerField(validators=[MaxValueValidator(140), MinValueValidator(5)])
    avatar = models.ImageField(default='Images/avatar.png')
    bio = models.TextField()
    user = models.OneToOneField(CustomUser, on_delete=models.CASCADE, related_name='profile_owner')
    ...
    def save(self, *args, **kwargs):
        old_file_name , file_ext = self.avatar.name.rsplit('.',1)
        self.avatar.name = CUtils.get_upload_path(self.__class__.__name__, self.user.username, file_ext)
        super().save(*args, **kwargs)
...
class Address(models.Model):
    ...
    profile = models.ForeignKey(Profile, on_delete=models.CASCADE, related_name='profile_address', null=True,blank=True)
...
class UserLastActivity(CModel.BaseClass):
    user = models.ForeignKey(CustomUser, on_delete=models.PROTECT, related_name='activitor')
